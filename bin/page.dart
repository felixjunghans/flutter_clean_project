...
const String BASE_PATH = 'lib/app/pages';
// base path to widget tests
const String TEST_BASE_PATH = 'test/widget_tests/pages';
...
void main(List<String> arguments) async {
  ...
  // absolute path to flutter_clean_project library
  final String libraryPath =
      path.join(path.dirname(Platform.script.toFilePath()), '../lib');

  if (packageName != null) {
    final parser = ArgParser()
      ..addOption('uiPath', defaultsTo: BASE_PATH, abbr: 'p');
    ...
    if (name == null) {
      ...
      if (await Directory(pathToScreen).exists()) {
        ...
        // generate files
        await copyAndRenameTemplate(Directory('$libraryPath/page_template'),
            Directory(pathToScreen), name);
      }
    }
  }
}
  ...
  await for (var entity in source.list(recursive: false)) {
    ...
    if (entity is Directory) {
      ...
      await file.replaceAll(
          'templateController', '${name.camelCase}Controller');
      await file.replaceAll('package:flutter_clean_project/page_template',
...
Future<void> generateTestFiles() async {}
